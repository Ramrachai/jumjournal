 ...
 */
if ( ! function_exists( 'understrap_post_meta' ) ) {
	function understrap_post_meta() {
		...
		if ( $post_date ) {
			printf( '<span class="post-time"><i class="far fa-calendar-alt"></i> ' . esc_html__( '%s', 'understrap' ) . '</span>', $post_date );
		}
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && understrap_categorized_blog() ) {
				/* translators: %s: Categories of current post */
				printf( '<span class="cat-links"> <i class="fas fa-map-marked-alt"></i> ' . esc_html__( '%s', 'understrap' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'understrap' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links"><i class="fas fa-tags"></i> ' . esc_html__( '%s', 'understrap' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		// comment number
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link"> <i class="far fa-comment-alt"></i> ';
			comments_popup_link( esc_html__( '0', 'understrap' ), esc_html__( '1', 'understrap' ), esc_html__( '%', 'understrap' ) );
			echo '</span>';
		}
	}
}